...
def drs_launch_settings_obj_builder(row):
    ...
    drs_launch_settings_obj.ec2LaunchTemplateID = row[5]
    drs_launch_settings_obj.targetInstanceTypeRightSizingMethod = row[6].upper()
    drs_launch_settings_obj.copyPrivateIp = str2bool(row[7])
    drs_launch_settings_obj.copyTags = str2bool(row[8])
    drs_launch_settings_obj.launchDisposition = row[9].upper()
    drs_launch_settings_obj.launchIntoInstanceProperties = {'launchIntoEC2InstanceID': ''}
    if row[10] != "":
        launch_into_instance_obj.launchIntoEC2InstanceID = row[10]
        drs_launch_settings_obj.launchIntoInstanceProperties = launch_into_instance_obj.__dict__
    byol_obj.osByol = str2bool(row[11])
